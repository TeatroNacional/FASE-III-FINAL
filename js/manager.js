...
const coleccionTeatro = firebase.firestore().collection('history');

    // Obtiene la descripción y las imágenes desde Firestore
    coleccionTeatro.doc('document_id').get().then((doc) => {
        if (doc.exists) {
            const data = doc.data();

            // Agrega la descripción al elemento con id "descripcion"
            document.getElementById('descripcion').innerHTML = `<p>${data.descripcion}</p>`;

            // Agrega el slider de imágenes al elemento con id "slider"
